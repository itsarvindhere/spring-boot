...
## ANOTHER WAY TO CREATE A REFERENCE

We can also used "mappedBy" in our "Student" entity class instead of using @JoinTable. So, we can simply write - 

    @ManyToMany(mappedBy = "students", cascade = {CascadeType.DETACH, CascadeType.MERGE, CascadeType.PERSIST, CascadeType.REFRESH})
    private List<Course> courses;

Moreover, there is no such rule that you have to write @ManyToMany with @JoinTable in the "Course" class only. Because in a "Many-to-Many" relationship, there is no owner side as such so you can also have the @JoinTable in the "Student" class and then "mappedBy" in the "Course" class.

In the case of "Many-to-Many" relationships in bidirectional scenario the Owner of the relationship can be selected arbitrarily, but having in mind the purpose you should select the entity that makes more sense to retrieve first or the one that is more used according to your purpose. 

You only need to remember that "Many" always need to be the owning side in "ManyToOne" scenarios.
